function [B, Ss, St, stat] = DGDTSC(Xs, Xt,label,num_bases, alpha,beta,gama,oumiga, lambda, mu, num_iters, Binit, pars)
% Transfer Sparse Coding (DGDTSC) Algorithm
%
%    minimize_B,S   0.5*||X - B*S||^2 + alpha*Tr(SLS') + mu*Tr(SMS') +gamma*Tr(SL^dS')+ lambda*sum(abs(S(:)))
...
%Construct the discriminative criterion Graph
%oumiga=0.8;
Ld=constructLd(X,oumiga,label);
...
while t < pars.num_iters
    ...
    if t ==1
        S= learn_scoding(B, X, alpha, mu,gama, pars.lambda/pars.sigma*pars.noise_var, L, M, Ld);
    else
        S= learn_scoding(B, X, alpha, mu,gama, pars.lambda/pars.sigma*pars.noise_var, L, M,Ld,S);
    end
    S(isnan(S))=0;
    Ss = S(:,1:ns);
    St = S(:,ns+1:ns+nt);
     ...
     % get objective
    [fobj, fresidue, fsparsity, flaplacian,fdiscriminative, fmmd] = getObjective(B, S, X, alpha, L, mu, M,gama,Ld, pars.noise_var, pars.lambda, pars.sigma);
    ...
    stat.fdiscriminative_total = stat.fdiscriminative_total+fdiscriminative;
    ...
    stat.fdiscriminative_avg(t) = stat.fdiscriminative_total/pars.nSmp;
    ...
    fprintf(['epoch= %d  fobj= %f  fresidue= %f  flaplacian= %f fdiscriminative=%f fmmd= %f  fsparsity= %f  elapsed_time= %0.2f ' ...
             'seconds\n'], t, stat.fobj_avg(t), stat.fresidue_avg(t), stat .flaplacian_avg(t),stat.fdiscriminative_avg(t),...
...
function [fobj, fresidue, fsparsity, flaplacian,fdiscriminative, fmmd] = getObjective(A, S, X, alpha, L, mu, M,gama,Ld, noise_var, lambda, sigma)
E = A*S - X;
fresidue  = 0.5/noise_var*sum(sum(E.^2));
flaplacian = 0.5*alpha.*trace(S*L*S');
fdiscriminative=0.5*gama.*trace(S*Ld*S');
fmmd = 0.5*mu.*trace(S*M*S');
fsparsity = lambda*sum(sum(abs(S/sigma)));
fobj = fresidue + fsparsity + fmmd + flaplacian+fdiscriminative;
end